 ...
 * @private
 */
function getPageTotal(resCalbk) {
    var sql = 'SELECT count(*) FROM tbl_articles',
        data = {};
    console.log('sql: ' + sql);
    pool.getConnection(function(err, connection) {
        // Use the connection
        connection.query(sql, function(err, results) {
            _errorHandler(err, results, resCalbk);
            data.pageTotal = Math.ceil(results[0]['count(*)'] / unit);
...
function _generateSuccessResJson(data) {
    var msg = 'Achieve data successfully/POST successfully';
    return {
        'success': true,
        'message': msg,
        'data': data
